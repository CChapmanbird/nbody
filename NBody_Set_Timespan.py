...
duration=8.45 #duration in seconds
precision  = 0.001 #precision in seconds
...
for ind,m in enumerate(mass):
    body, = ax1.plot([pos[0,ind,0]],[pos[0,ind,1]],'o',color=colour_matrix[ind],
                     markersize=12*m,zorder=3)
    bodies.append(body)
    line, = ax1.plot([pos[0,ind,0]],[pos[0,ind,1]],color=colour_matrix[ind],
                     zorder=-1,linewidth=3,alpha=0.5)
    lines.append(line)
com, = ax1.plot([0],[0],'r+',markersize=20 )
time_text = plt.text(0.02,0.89,'hi',fontsize=32,transform=ax1.transAxes)
...
def animate_phase_timespan(k):
    global sim_time2
    sim_time2= sim_time2+ (0.01/precision)*dt
    global prev_time2
    global pos
    global vel
    ...
    times.append(time.time()-prev_time2)
    prev_time2 = time.time()\
    ...
    time_text2.set_text("Time: {0:.2f}s".format(sim_time2)) #\n{0:.1f}d".format(sim_time/86400))
#    fps_text2.set_text("FPS: {0:.1f}".format(1/np.mean(times[-5:])))

if show_phase==True:
    ani2=animation.FuncAnimation(fig2,animate_phase_timespan,frames=range(0,int(duration/precision-1),int(0.01/precision)), interval=15)
    ani2.save('phasetest.mp4',writer=writer)
if show_pos==True:
